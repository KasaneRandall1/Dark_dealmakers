...
class rules():
    ...
    def __rule_1(self):
        ...
        rule_1_hp = self.font.render("Your life points represent your survival.",True,self.red)
        rule_1_play_cost = self.font.render("Playing a card costs life points equal to the card's value. Choose carefully.",True,self.red)
        rule_1_death = self.font.render("If your life points reach zero, you lose the game.",True,self.red)
    ...
    def __rule_2(self):
        ...
        rule_2_turn = self.font.render("You can only interact on your turn.",True,self.red)
        rule_2_attack = self.font.render("You choose the target for the monster you want to attack with.",True,self.red)
    ...
    def __misc(self):
        ...
        misc_2 = self.font.render("You can't see ahead on the map since you walk through the Abyss of Death",True,self.red)
        misc_2_1 = self.font.render("You can choose from 4 paths without seeing what is ahead of the path.",True,self.red)
        misc_3 = self.font.render("Your Goal is to beat Death, so you can get to the world of the living.",True,self.red)
        misc_4 = self.font.render("Beware the Abyss is full of grotesque monster.",True,self.red)
...
class prologue():
    ...
    def show_prologue(self):
        self.display.fill((0,0,0))
        txt_1 = self.font.render("In the forgotten corners of reality lies the Abyss, a realm teeming with eldritch horrors,",True,self.red)
        txt_2 = self.font.render("grotesque monster and undescribeable things that defy the feeble understanding of mortals.",True,self.red)
        txt_3 = self.font.render("Whispers of ancient evils echo through the void,",True,self.red)
        txt_4 = self.font.render("drawing the daring and the desperate to its shadowy depths.",True,self.red)
        txt_5 = self.font.render("Death is giving you a chance to escape the Abyss.",True,self.red)
        txt_6 = self.font.render("The Offer itself is twisted. You gain the power to summon whatever is in the Abyss,",True,self.red)
        txt_7 = self.font.render("but you must sacrifice a part of you Life.",True,self.red)
        txt_8 = self.font.render("Will you make it to the End or will you succumb to the Abyss?",True,self.red)
...
class combat_gui():
    def __init__(self,screen) -> None:
        ...
        self.player_card_field = [self.card_1,self.card_2,self.card_3,self.card_4,self.card_5]
        ...
        self.cpu_card_field = [self.c_field_1,self.c_field_2,self.c_field_3,self.c_field_4,self.c_field_5]
    ...
    def __create_player_cards(self):
        ...
        field_card_pos = [self.card_1.topleft,self.card_2.topleft,self.card_3.topleft,self.card_4.topleft,self.card_5.topleft]
        hand_card_pos = [self.h_card_1.topleft,self.h_card_2.topleft,self.h_card_3.topleft,self.h_card_4.topleft]
    ...
    def __create_cpu_cards(self):
        field_card_img = []
        c_field_pos = [self.c_field_1.topleft,self.c_field_2.topleft,self.c_field_3.topleft,self.c_field_4.topleft,self.c_field_5.topleft]
...
class death_screen():
    def __init__(self,screen) -> None:
        ...
        self.death_msg = self.font.render("YOU      SUCCUMB     TO      DEATH",True,(255,0,0))
